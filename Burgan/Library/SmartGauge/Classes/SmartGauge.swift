...
public class SmartGauge: UIView {
    ...
    private func setupBaseLayers() {
        ...
        let gaugeHolderFrame = CGRect(x: 40, y: gaugeYVal, width: layer.bounds.width * multiplier, height: 250)
    ...
    private func setupTrackLayer() {
        ...
        trackLayer?.gaugeValue = (gaugeValue ?? 0.0) > 99 ? (gaugeValue ?? 0.0) : ((gaugeValue ?? 0.0) - 3.5)
