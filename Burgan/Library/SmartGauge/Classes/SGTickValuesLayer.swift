...
class SGTickValuesLayer: SGBaseLayer {
    ...
    private func setupTickValues() {
        ...
        for index in 0..<(labelsCount+1) {
            ...
            if screenWidth < 321{
            layer.frame = bounds.insetBy(dx: +radius/3.5, dy: +radius/3.5 + 32)
            }else if screenWidth < 376 && screenWidth > 320{
                layer.frame = bounds.insetBy(dx: +radius/3.5, dy: +radius/3.5 + 10)
            }else if screenWidth < 415 && screenWidth > 375{
                layer.frame = bounds.insetBy(dx: +radius/3.5, dy: +radius/3.5 + 2)
            }
            else{
