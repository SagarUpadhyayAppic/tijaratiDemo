...
open class WWCalendarTimeSelector: UIViewController, UITableViewDelegate, UITableViewDataSource, WWCalendarRowProtocol, WWClockProtocol {
    ...
    open override func viewDidLoad() {
        ...
        yearRow1 = Int(optionCurrentDate.yearr)! - 5
        optionSelectionType = .range
        selMultipleDatesTable.isHidden = optionSelectionType != .multiple
        backgroundSelView.isHidden = optionSelectionType != .single
        backgroundRangeView.isHidden = optionSelectionType != .single
    ...
    @IBAction func selectMonth(_ sender: UIButton) {
        ...
        if delegate?.WWCalendarTimeSelectorShouldSelectDate?(self, date: date) ?? true {
            optionCurrentDate = optionCurrentDate.change(year: Int(date.yearr), month: date.monthd, day: date.day).beginningOfDay
            updateDate()
        }
    }
    ...
    fileprivate func updateDate() {
        ...
        delegate?.WWCalendarTimeSelectorDone?(self, dates: optionCurrentDateRange.array)
        if shouldResetRange {
    ...
    open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if tableView == calendarTable {
            ...
            let currentYear = Int(Date().yearr)
            let displayYear = yearRow1 + (indexPath as NSIndexPath).row
            if displayYear > currentYear! {
                cell.textLabel?.font = Int(optionCurrentDate.yearr) == displayYear ? optionCalendarFontFutureYearsHighlight : optionCalendarFontFutureYears
                cell.textLabel?.textColor = Int(optionCurrentDate.yearr) == displayYear ? optionCalendarFontColorFutureYearsHighlight : optionCalendarFontColorFutureYears
            }
            else if displayYear < currentYear! {
                cell.textLabel?.font = Int(optionCurrentDate.yearr) == displayYear ? optionCalendarFontPastYearsHighlight : optionCalendarFontPastYears
                cell.textLabel?.textColor = Int(optionCurrentDate.yearr) == displayYear ? optionCalendarFontColorPastYearsHighlight : optionCalendarFontColorPastYears
            }
            else {
                cell.textLabel?.font = Int(optionCurrentDate.yearr) == displayYear ? optionCalendarFontCurrentYearHighlight : optionCalendarFontCurrentYear
                cell.textLabel?.textColor = Int(optionCurrentDate.yearr) == displayYear ? optionCalendarFontColorCurrentYearHighlight : optionCalendarFontColorCurrentYear
            }
        ...
        if row == 1 {
            ...
            for _ in startRow..<row {
                if rowType == .month {
                    ...
                    if newStartDate.monthd != startDate.monthd {
                        rowType = .month
                    }
    ...
    internal func WWCalendarRowDidSelect(_ date: Date) {
        if delegate?.WWCalendarTimeSelectorShouldSelectDate?(self, date: date) ?? true {
            switch optionSelectionType {
            case .single:
                optionCurrentDate = optionCurrentDate.change(year: Int(date.yearr), month: date.monthd, day: date.day)
                updateDate()
...
protocol WWCalendarRowProtocol: NSObjectProtocol {
        ...
        if detail.type == .month {
            ...
            for i in 1...7 {
                if date.weekday == i {
                    ...
                    date = date + 1.day
                    if date.monthd != startDate.monthd {
                        break
                    }
                }
            }
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if detail.type == .date {
            ...
            if let touch = touches.sorted(by: { $0.timestamp < $1.timestamp }).last {
                ...
                if dateTapped.monthd == detail.startDate.monthd && delegate.WWCalendarRowDateIsEnable(dateTapped){
